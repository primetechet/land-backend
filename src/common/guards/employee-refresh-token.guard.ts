...
export class EmployeeRefreshTokenGuard implements CanActivate {
  private readonly logger = new Logger(EmployeeRefreshTokenGuard.name);
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // This prevents conflicts with the global EmployeeAuthGuard
    ...
    try {
      ...
      this.logger.warn(
        `Employee refresh token validation failed: ${error.message}`,
        {
          error: error.name,
